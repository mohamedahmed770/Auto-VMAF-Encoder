    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=SETTINGS.num_parallel_vmaf_runs) as executor:
            ...
            for iteration in range(SETTINGS.max_iterations):
                ...
                step = (high_cq - low_cq) / (SETTINGS.num_parallel_vmaf_runs + 1)
                test_cqs = sorted(list(set([
                    round(low_cq + (j+1) * step)
                    for j in range(SETTINGS.num_parallel_vmaf_runs)
                    if low_cq <= round(low_cq + (j+1) * step) <= high_cq
                    and round(low_cq + (j+1) * step) not in tested_cqs
                ])))
                
                # Always include boundaries if not tested
                if low_cq not in tested_cqs and low_cq not in test_cqs:
                    test_cqs.insert(0, low_cq)
                if high_cq not in tested_cqs and high_cq not in test_cqs and len(test_cqs) < SETTINGS.num_parallel_vmaf_runs:
                    test_cqs.append(high_cq)

                if not test_cqs:
                    if high_cq - low_cq > 1:
                        test_cqs = [round((low_cq + high_cq) / 2)]
                    else:
                        break

                log_callback(task_id, f"Iteration {iteration + 1}/{SETTINGS.max_iterations}: Testing CQ/CRFs {test_cqs}")

                
                progress_tracker = IterationProgressTracker()
                iter_task_id = vmaf_progress.add_task(f"VMAF Iteration {iteration + 1}", total=100)
                progress_tracker.initialize(vmaf_progress, iter_task_id, len(test_cqs))

                
                futures = {}
                for i, cq in enumerate(test_cqs):
